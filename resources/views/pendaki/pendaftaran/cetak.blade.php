...
<head>
    ...
    <title>Bukti Pendaftaran - {{ $pendaftaran->id_booking }}</title>
    <style>
...
<body>
    <div class="container">
        ...
        <div class="card">
            ...
            <div class="grid">
                <div class="grid-item"><strong>ID Booking:</strong> {{ $pendaftaran->id_booking }}</div>
                <div class="grid-item"><strong>Status:</strong> <span class="status-badge">DISETUJUI</span></div>
                <div class="grid-item"><strong>Nama Ketua:</strong> {{ $pendaftaran->nama_ketua }}</div>
                <div class="grid-item"><strong>Tanggal Pendaftaran:</strong> {{ $pendaftaran->created_at->format('d/m/Y H:i') }}</div>
                <div class="grid-item"><strong>Gunung:</strong> {{ $pendaftaran->gunung->nama_gunung }}</div>
                <div class="grid-item"><strong>Jalur:</strong> {{ $pendaftaran->gunung->jalur }}</div>
                <div class="grid-item"><strong>Tanggal Pendakian:</strong> {{ date('d/m/Y', strtotime($pendaftaran->tanggal_pendakian)) }}</div>
                <div class="grid-item"><strong>Jumlah Anggota:</strong> {{ $pendaftaran->jumlah_anggota }} orang</div>
            </div>
        </div>
        ...
        <div class="card">
            ...
            <table class="table">
                ...
                <tbody>
                    @foreach($pendaftaran->anggota as $i => $a)
                    <tr>
                        <td>{{ $i+1 }}</td>
                        <td>{{ $a->nama }}</td>
                        <td>{{ $a->no_ktp }}</td>
        ...
        <div class="footer">
            <p class="no-print" style="margin-bottom: 16px;">
                ...
                <a href="{{ route('pendaki.pendaftaran.show', $pendaftaran->id) }}" class="no-print" style="margin-left: 12px; color: #2563eb; text-decoration: none;">← Kembali</a>
            </p>
