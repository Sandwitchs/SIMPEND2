    ...
    <div class="card" style="width: 100%; max-width: 460px; position: relative; z-index: 2; border: 1px solid rgba(255, 255, 255, 0.1); box-shadow: 0 20px 50px rgba(0, 0, 0, 0.6); margin: 40px auto;">
        ...
        <form method="POST" action="{{ route('register.post') }}">
            ...
            <div class="form-group">
                ...
                <input type="text" name="name" value="{{ old('name') }}" placeholder="Nama Lengkap sesuai KTP" required autocomplete="name" autofocus>
            </div>
            
            <div class="form-group">
                <label>Email</label>
                <input type="email" name="email" value="{{ old('email') }}" placeholder="contoh@domain.com" required autocomplete="email">
            </div>
        ...
        <div style="text-align: center; margin-top: 24px; border-top: 1px solid rgba(255, 255, 255, 0.06); padding-top: 20px; font-size: 14px; color: #9ca3af;">
            Sudah punya akun? <a href="{{ route('login') }}" style="color: #10b981; font-weight: 700; text-decoration: none;">Login</a>
        </div>
    </div>
</div>
@endsection
